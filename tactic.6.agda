...
-}
 ...
 build {V} {i} {j} {k} (p $ p₁) q = let s = build p₁ q
                                        t = build p (canon s)
                                    in
                                     < flat t ■ flat s , canon t , ((flat t ■ flat s) ■ flatten' q ≡⟨ (■-assoc (flat t) (flat s) (flatten' q))⁻¹ ⟩
                                     flat t ■ flat s ■ flatten' q ≡⟨ ap (λ Q → (flat t) ■ Q) (prf s) ⟩
                                     flat t ■ flatten' (canon s) ≡⟨ prf t ⟩
                                     flatten' (canon t)
                                     ▻)>
                                     where open cert'

 proof : {V : Set} → {i j : V} → (p : Path V i j) → (q : Path V i j) → (cert'.canon (build {V} {i} {j} p nil) ≡ cert'.canon (build q nil)) → cert'.flat (build p nil) ≡ cert'.flat (build q nil)
 proof {V} {i} {j} p q r =
               flat (build p nil) ≡⟨ p≡p■refl ⟩
               flat (build p nil) ■ refl ≡⟨ prf (build p nil) ⟩
               flatten' (canon (build p nil)) ≡⟨ ap flatten' r ⟩
               flatten' (canon (build q nil)) ≡⟨ (prf (build q nil))⁻¹ ⟩
               flat (build q nil) ■ refl ≡⟨ p≡p■refl ⁻¹ ⟩
               flat (build q nil)
               ▻
               where open cert'
