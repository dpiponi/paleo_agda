...
-- ppp'' = refl
 ...
 d₃ _ _ r = j (λ x w (r : x ≡ w) → refl' x ■ (refl' x ■ r) ≡ (refl' x ■ refl' x) ■ r)
              d₄
              r
              

 d₂ : {A : Set} → (x z : A) → (q : x ≡ z) → (w : A) → (r : z ≡ w) → (refl' x ■ (q ■ r)) ≡ ((refl' x ■ q) ■ r)
 d₂ _ _ q = j (λ x z (q : x ≡ z) → (w : _) → (r : z ≡ w) → (refl' x ■ (q ■ r)) ≡ ((refl' x ■ q) ■ r))
              d₃
              q

 d₁ : {A : Set} → (x y : A) → (p : x ≡ y) → (z : A) → (q : y ≡ z) → (w : A) → (r : z ≡ w) → (p ■ (q ■ r)) ≡ ((p ■ q) ■ r)
 d₁ _ _ p = j (λ x y (p : x ≡ y) → (z : _) → (q : y ≡ z) → (w : _) → (r : z ≡ w) → (p ■ (q ■ r)) ≡ ((p ■ q) ■ r))
              d₂
              p
{-
