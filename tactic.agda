...
-- Lift ends of p to f x and f y. Transport f x along p. You should get f y.
 ...
 build : {V : Set} → {i j k : V} → (p : Path V i j)
         → (q : List V j k)
         → cert' i j k q
 build (REFL) q = < refl , q , p≡refl■p ⁻¹ >
 build (con e) q = < e , cons e q , refl >
 build (p □ p₁) q = let s = build p₁ q
                        t = build p (canon s)
                                    in
...
{-
 gid' : {V : Set} → {i k : V} → (q : List V i k) → cert' i i k q
 gid' q = < refl , q , (refl ■ flatten' q ≡⟨ p≡refl■p ⁻¹ ⟩
                                       flatten' q
                                       ▻)>

 con' : {V : Set} → {i j k : V} (e : i ≡ j) → (q : List V j k) → cert' i j k q
 con' e q = < e , cons e q , refl >

 _□'_ : {V : Set} → {i j k l : V} → (p : cert' i j k q) → (p₁ : cert' j k l) → (q : List V k l) → cert' i k l q
 (p □' p₁) q = let s = build p₁ q
                   t = build p (canon s)
                in
                                     < flat t ■ flat s , canon t , ((flat t ■ flat s) ■ flatten' q ≡⟨ (■-assoc (flat t) (flat s) (flatten' q))⁻¹ ⟩
                                     flat t ■ flat s ■ flatten' q ≡⟨ ap (λ Q → (flat t) ■ Q) (prf s) ⟩
...
-}
 ...
 test : {A : Set} → {x y : A} → (p : x ≡ x) → ((p ■ p) ■ p) ■ refl ≡ p ■ refl ■ (p ■ p)
 test p = let p' = con p in
                proof (((p' □ p') □ p') □ REFL) (p' □ REFL □ (p' □ p'))
                refl
