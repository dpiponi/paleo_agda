...
-- Lift ends of p to f x and f y. Transport f x along p. You should get f y.
 ...
 build : {V : Set} → {i j k : V} → (p : Path V i j)
         → (q : List V j k)
         → ∑ (List V i k) (λ l → flatten p ■ flatten' q ≡ flatten' l) -- canonical , main
 build {V} {.j} {j} (gid .j) q = q , (refl ■ flatten' q ≡⟨ p≡refl■p ⁻¹ ⟩
                                       flatten' q
                                       ▻)
 build (con e) q = (cons e q , refl)
 build {V} {i} {j} {k} (p $ p₁) q = let s = build p₁ q
                                        t = build p (pr₁ s)
                                    in
                                     (pr₁ t , ((flatten p ■ flatten p₁) ■ flatten' q ≡⟨ (■-assoc (flatten p) (flatten p₁) (flatten' q))⁻¹ ⟩
                                     flatten p ■ flatten p₁ ■ flatten' q ≡⟨ ap (λ Q → flatten p ■ Q) (pr₂ s) ⟩
                                     flatten p ■ flatten' (pr₁ s) ≡⟨ pr₂ t ⟩
                                     flatten' (pr₁ t)
                                     ▻))

 proof : {V : Set} → {i j : V} → (p : Path V i j) → (q : Path V i j) → (pr₁ (build {V} {i} {j} p nil) ≡ pr₁ (build q nil)) → flatten p ≡ flatten q
 proof {V} {i} {j} p q r =
               flatten p ≡⟨ p≡p■refl ⟩
               flatten p ■ flatten' nil ≡⟨ pr₂ (build p nil) ⟩
               flatten' (pr₁ (build p nil)) ≡⟨ ap flatten' r ⟩
               flatten' (pr₁ (build q nil)) ≡⟨ (pr₂ (build q nil))⁻¹ ⟩
